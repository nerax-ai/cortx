...
- **执行轮廓：** 本轮先实现 core runtime 的通用 `ctx.register(type, id, factory)` 类型映射与兼容编译层，让 `agent.*` 注册可以在无 TUI/Web 的 headless core 中真实运行；更完整的 tool rewrite、session policy、asset pack schema 留给后续。
- **停止条件：** 新旧插件注册方式都能通过 registry 装载并影响 agent loop，相关 SDK/core 类型编译通过，核心测试覆盖 `agent.tool`、system/messages transform、tool before/after、error/context/event observer 的兼容路径。
...
- 所有 hook 都堆在 `CortxPlugin` 这个大对象里，命名偏实现细节，长期不利于扩展。
- `system.transform` 和 `messages.transform` 都叫 `transform` 时，读插件代码要靠 hook 名才能知道语义。
...
Cortx core 应采用类似 OpenCode 的务实分层，但比 OpenCode 的 hook bag 更类型化：
...
| 区域 | 当前扩展点 | 状态 | 代码位置 | 用途 |
|---|---|---|---|---|
| Agent runtime | `CortxPlugin.tools` | 已实现 | `packages/sdk/src/index.ts`, `packages/core/src/loop.ts` | 增加 agent 可调用工具。 |
| Agent runtime | `CortxPlugin["system.transform"]` | 已实现 | `packages/sdk/src/index.ts`, `packages/core/src/loop.ts` | 修改 system prompt。 |
| Agent runtime | `CortxPlugin["messages.transform"]` | 已实现 | `packages/sdk/src/index.ts`, `packages/core/src/loop.ts` | 修改 outgoing messages。 |
| Tool runtime | `CortxPlugin["tool.execute.before"]` | 已实现 | `packages/sdk/src/index.ts`, `packages/core/src/loop.ts` | 工具执行前 gate、rewrite、skip。 |
| Tool runtime | `CortxPlugin["tool.execute.after"]` | 已实现 | `packages/sdk/src/index.ts`, `packages/core/src/loop.ts` | 工具结果后处理。 |
| Recovery | `CortxPlugin["error.recover"]` | 已实现 | `packages/sdk/src/index.ts`, `packages/core/src/loop.ts` | stream/provider error retry policy。 |
| Recovery | `CortxPlugin["context.overflow"]` | 已实现 | `packages/sdk/src/index.ts`, `packages/core/src/loop.ts` | context overflow 后压缩 messages。 |
| Observation | `CortxPlugin["event"]` | 已实现 | `packages/sdk/src/index.ts`, `packages/core/src/loop.ts` | 观察 canonical `AgentEvent`。 |
...
- R2. 当前 `CortxPlugin` 必须保留兼容，但文档和新注册 API 应使用正式 extension type 名称。
...
ctx.register('agent.systemTransform', 'repo-policy', () => ({
  async transformSystem(input, ctx) {
    return { system: `${input.system}\n\n${await ctx.assets.readText('repo-policy.md')}` };
  },
}));
```
...
`agent.tool` 是给 agent 增加可调用能力的主入口。它对应当前 `CortxPlugin.tools` 和 `CortxConfig.tools`。
...
ctx.register('agent.tool', 'browser.open', () => ({
  ...
  async execute(input, ctx) {
    ...
    const page = await browser.open(String(input.url), { signal: ctx.abortSignal });
...
`agent.systemTransform` 修改每次模型调用前的 system prompt。它对应当前 `system.transform`。
...
ctx.register('agent.systemTransform', 'repo-policy', () => ({
  async transformSystem(input, ctx) {
    const policy = await ctx.assets.readText('.cortx/policy.md').catch(() => '');
...
`agent.messagesTransform` 在模型调用前重写 outgoing messages。它对应当前 `messages.transform`。
...
`agent.toolBefore` 在工具真正执行前运行。它对应当前 `tool.execute.before`，但能力应从 `skip/result` 扩展为 allow/deny/rewrite/short-circuit。
...
`agent.toolAfter` 在工具返回后、结果进入模型上下文前运行。它对应当前 `tool.execute.after`。
...
```ts
ctx.register('agent.toolAfter', 'result-budget', () => ({
  async afterToolExecute(input, ctx) {
    const text = stringifyToolOutput(input.result);
    if (text.length <= 8000) return { result: input.result };

    const artifact = await ctx.artifacts.writeText('tool-output.txt', text);
    return {
      result: {
        ...input.result,
...
`agent.errorRecover` 处理模型 stream error 的恢复策略。它对应当前 `error.recover`。
...
ctx.register('agent.errorRecover', 'transient-retry', () => ({
  async recoverError(input) {
    ...
    if (input.error.code === 'stream_error' && input.attempt < 2) {
...
`agent.contextOverflow` 处理 context window 超限。它对应当前 `context.overflow`。
...
ctx.register('agent.contextOverflow', 'compact-history', () => ({
  async handleContextOverflow(input, ctx) {
    const summary = await ctx.model.summarize(input.messages.slice(0, -8));
    return {
      messages: [
...
- no recovery 或 recovery null 时发 `context_overflow` 后发 terminal `error`。
- tool-call/tool-result pairing 不被压缩破坏。

### 8. `agent.eventObserver`

`agent.eventObserver` 观察 canonical `AgentEvent` stream。它对应当前 `event` hook。
...
```ts
ctx.register('agent.eventObserver', 'event-tape', () => ({
  async onAgentEvent(event, ctx) {
    await ctx.storage.append('events.jsonl', JSON.stringify(event) + '\n');
  },
}));
```
  ...
  setup(ctx) {
    ...
    ctx.register('agent.eventObserver', 'metrics', () => ({
      onAgentEvent(event, ctx) {
        ctx.metrics.count(`agent.event.${event.type}`);
      },
    }));
  },
});
...
底层仍可映射到当前 `CortxPlugin`：

```ts
const legacyPlugin: CortxPlugin = {
  tools: collectedTools,
  'system.transform': runSystemTransforms,
  'messages.transform': runMessageTransforms,
  'tool.execute.before': runToolBefore,
  'tool.execute.after': runToolAfter,
  'error.recover': runErrorRecover,
  'context.overflow': runContextOverflow,
  event: runEventObservers,
};
```

这样可以兼容旧代码，同时保持注册模型足够通用。
...
### Phase 1: Document and Stabilize Current Core

- Freeze current `CortxPlugin` behavior in conformance tests.
- Add docs mapping current hook names to proposed `agent.*` names.
- Clarify skills as assets, not runtime extension points.

### Phase 2: Add Typed Registration Layer

- Implement `defineCortxPlugin()` and typed `ctx.register()` maps for `agent.*`.
- Internally compile typed contributions to legacy `CortxPlugin`.
...
### Phase 3: Expand Missing Core Semantics

- Extend `toolBefore` result from `{ skip, result }` to allow/rewrite/shortCircuit.
- Add explicit plugin ordering metadata.
- Add failure isolation policy per extension type.
- Add optional `agent.sessionPolicy`.

### Phase 4: Asset Models
...
### Phase 5: Official Plugin Fixtures
...
- KTD2. This slice compiles new `agent.*` contributions into the existing `CortxPlugin` hook bag. The loop keeps its stable internal contract while plugin authors get semantic extension names and contribution methods.
- KTD3. Legacy `cortx` extensions remain valid. Registry resolution first preserves configured legacy extensions, then discovers matching `agent.*` contributions by extension id, full id, or package id.
...
- KTD5. `agent.toolBefore` gets the first real semantic upgrade. The compatibility layer supports legacy `skip/result` plus `allow`, `rewrite`, `deny`, and `shortCircuit` results, while broader permission and session policy behavior remains follow-up work.
...
flowchart TB
  ...
  C --> E
  E --> F["legacy cortx extension"]
  E --> G["agent.* contributions"]
  G --> H["compile to CortxPlugin hook bag"]
  F --> I["agentLoop plugins"]
  H --> I
  I --> J["system/messages/tool/error/context/event hooks"]
```
...
- **Approach:** Add stable `agent.*` constants for tool, system transform, messages transform, tool before, tool after, error recovery, context overflow, and event observer. Keep `cortx` as the legacy extension type. Define contribution interfaces with semantic methods and typed input/output records. Export `defineCortxPlugin()` as a thin typed identity helper around inline plugins.
- **Patterns to follow:** `packages/tui/src/types/tui-plugin.ts` for typed extension constants and factory map; existing `CortxPlugin` and `Tool` types in `packages/sdk/src/index.ts`.
...
### U2. Core registry resolver and compatibility compiler

- **Goal:** Let configured plugins registered through `agent.*` run inside `Cortx` by compiling them into the current `CortxPlugin` loop contract while preserving legacy `cortx` plugins.
...
- **Approach:** Change `CortxPluginRegistry` to the new Cortx factory map. Resolve each configured `{ use }` by scanning registry extensions for matching id/full id/package id. Create legacy `cortx` extensions directly. Create matching `agent.*` contributions, group them, and return one compatibility `CortxPlugin` with hook functions that call semantic contribution methods in registry order.
- **Patterns to follow:** `packages/tui/src/tui-registry.ts` for `listExtensions(type)` plus `create(type, fullId, instanceId, options, namespace)`; current `resolvePlugins()` in `packages/core/src/agent.ts`.
- **Test scenarios:** A legacy `cortx` event plugin still observes `done`. An `agent.eventObserver` plugin observes `done` when configured by extension id. A package-level config entry activates multiple contributions from one plugin package. Missing configured extension still returns the registry's not-found error.
...
- **Approach:** Extend the legacy before result shape without breaking `skip/result`. Support `rewrite` by changing the actual tool-call input before `tool.execute`. Support `deny` and `shortCircuit` by emitting a structured tool result without executing the tool. Compile `agent.toolAfter` to the existing after hook so model-visible results are normalized before message append.
- **Patterns to follow:** Current `tool.execute.before` skip handling and `runToolCall()` result formatting in `packages/core/src/loop.ts`.
- **Test scenarios:** A registered `agent.tool` appears in the model tool list and executes. `agent.toolBefore` rewrites input and the tool receives the rewritten value. `agent.toolBefore` short-circuits and the tool body is not called. `agent.toolAfter` modifies the final model-visible output. Legacy `tool.execute.before` skip still works.
- **Verification:** Tool-focused core tests pass with both legacy and new contribution forms.
...
### U5. Public exports and compatibility cleanup
...
- **Approach:** Re-export the new SDK extension constants and types from core. Update local registry construction in server/TUI/tests to use the Cortx factory map or cast through the exported `CortxPluginRegistry` where it shares a Synax registry instance.
- **Patterns to follow:** Existing core SDK re-export pattern in `packages/core/src/index.ts`.
- **Test scenarios:** Server session-manager tests still pass with legacy registry plugins. TUI language registry typing compiles where Cortx and Synax share the same `PluginRegistry` instance.
...
|---|---|---|
| `bun test packages/core/tests/core-extensions.test.ts` | U1-U4 | New `agent.*` registry and compatibility behavior passes. |
| `bun test packages/core/tests/agent-background.test.ts packages/server/tests/server.test.ts` | U2, U5 | Legacy registry compatibility remains intact through core and server sessions. |
| `bun test packages/core/tests/loop.test.ts` | U3, U4 | Existing loop hook behavior still passes after before-result and observer changes. |
...
- `packages/sdk/src/index.ts` 定义当前 `CortxPlugin`、`Tool`、`ToolContext`、`AgentEvent`。
